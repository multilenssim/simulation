...
def build_side(E, n, d, t, blockers=True, b=1.0/1000):
    ...
    if blockers:
        ...
        for theta in np.linspace(3*np.pi/6, 11*np.pi/6, 3):
            face = face + Solid(mh.shift(corner_blocker(R, blocker_thickness), (2*R*(n-1/3.0)*np.cos(theta), 2*R*(n-1/3.0)*np.sin(theta), 0)), lm.lensmat, lm.ls, black_surface, 0xff0000)
        ...
        for i in np.linspace(0, 3*n*(n-1)/2, (3*n**2-3*n+2)/2):
            for j in np.linspace(1, 11, 6): 
                face = face + Solid(mh.shift(mh.rotate(inner_blocker(R, blocker_thickness, 128), make_rotation_matrix(j*np.pi/6, (0, 0, 1))), (xcoords[i] + 7*np.sqrt(3)/12*R*np.cos(3*np.pi/2-j*np.pi/6), ycoords[i]+7*np.sqrt(3)/12*R*np.sin(3*np.pi/2-j*np.pi/6), 0)), lm.lensmat, lm.ls, black_surface, 0xff0000)

            if d < 1:
                face = face + Solid(mh.shift(mh.rotate(cylindrical_shell(d*R, R, blocker_thickness), make_rotation_matrix(np.pi/2, (1,0,0))), (xcoords[i], ycoords[i], 0)), lm.lensmat, lm.ls, black_surface, 0xff0000)
