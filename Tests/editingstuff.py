...
def build_pmt_icosahedron(edge_length, base, diameter_ratio, thickness_ratio):
    ...
    side_length = 2*np.sqrt(3)/phi**2*focal_length + edge_length
    for k in range(20):
        kabamland.add_pmt(Solid(triangle_mesh(side_length, thickness), glass, lm.ls, lm.fulldetect, 0xBBFFFFFF), rotation=np.dot(make_rotation_matrix(spin_angle[k], direction[k]), make_rotation_matrix(angle[k], axis[k])), displacement=focal_length*normalize(facecoords[k])+facecoords[k]+thickness/2.0)

def build_kabamland(edge_length, base, diameter_ratio, thickness_ratio, blockers=True, blocker_thickness_ratio=1.0/1000):
    build_lens_icosahedron(edge_length, base, diameter_ratio, thickness_ratio, blockers, blocker_thickness_ratio)
    build_pmt_icosahedron(edge_length, base, diameter_ratio, thickness_ratio)
