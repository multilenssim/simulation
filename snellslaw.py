...
# def find_main_deviation(target, coordarray):
#      #keeps target positions that are the closest to the centroid of the entire coordarray, and then finds the deviation of those remaining positions. THIS ASSUMES FOR THE CALCULATION OF DEVIATION THAT ALL POSITIONS LAND AT THE SAME Y POSITION.
#     centroid = np.mean(coordarray, 0)
#     radius = perf_radius(target, coordarray)
...
if __name__ == '__main__':
    ...
    #print 'flat surface snells law test:' + str(snells_law_test(np.pi/5))    
    ...
    ##2.25 = pc_focal_length(0.75)
    ##3.00 = pc_focal_length(1.00)
    ...
    lensc = cylindrical_pclens(pcrad, diameter, 1.0)
    ...
    pmt = Solid(make.box(5.0,1.0,5.0), lm.lensmat, lm.lensmat, surface=lm.fulldetect, color=0x00ff00)
    ...
    def find_perf_radii(angle, pmtpos, goal, shape):
        ...
        for ev in sim.simulate(single_ray([xpos, 5, 0], [0, -1, 0], 20), keep_photons_beg=True, keep_photons_end=True, run_daq=True, max_steps=1000):
            ...
            print 'angle quotient: t2/pt2', final_angle/predicted_final_angle
...
print best_pmt_position(0, 8, 0.7, 'pc')
