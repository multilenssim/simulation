...
class LensSys(object):
    ...
    def get_scale_factor(self, scale_rad):
        ...
        sf = scale_rad/self._sys_rad
        logger.warning('Lens system %s scale factor: %f, %f, %f' % (self.name, sf, scale_rad, self._sys_rad))
        return sf

    def get_half_EPD(self, scale_rad, EPD_ratio):
        # Returns the radius of the entrance pupil, as a fraction EPD_ratio of the first lens
        # radius, scaled up appropriately
        scale_factor = self.get_scale_factor(scale_rad)

        # This change makes the scale factor consistent throughout
        '''
        sys_rad_unscaled = self._sys_rad
        sys_rad = scale_factor*sys_rad_unscaled
        return sys_rad*EPD_ratio
        '''
        lens1_rad_unscaled = self._lens1_rad
        lens1_rad = scale_factor*lens1_rad_unscaled
        return lens1_rad*EPD_ratio


    def get_system_measurements(self, scale_rad):
        ...
        scale_factor = self.get_scale_factor(scale_rad)
        fsp_unscaled = self._location_of_focal_surface if hasattr(self, '_location_of_focal_surface') else self._focal_length
        det_r_unscaled = self._detector_r_curve
    ...
    def get_lens_mesh_list(self, scale_rad):
        # Returns a list of lens meshes for the given lens_system_name, scaled to match scale_rad
        lenses = []
        scale_factor = self.get_scale_factor(scale_rad)
        logger.warning('Scale factor for lens %s: %f' % (self.name, scale_factor))
        if self.name == 'Jiani3':
            # Asphere - Jiani design 3
            as_rad = self._lens1_rad*scale_factor
        ...
        elif self.name == 'Sam1-old':

            # First lens
            lens1rad = self._lens1_rad * scale_factor
            ...
            # Second lens
            lens2rad = self._sys_rad * scale_factor
            ...
            d2_unscaled = self._t1 - thickness_of_lens_curved_part(self._c1, self._lens1_rad) + self._gap + thickness_of_lens_curved_part(self._c3, self._lens2_rad)
            logger.info('Unscaled and scaled d2: %f, %f' % (d2_unscaled, d2_unscaled*scale_factor))

            distance_to_lens_2 = self._t1 - thickness_of_lens_curved_part(self._c1, self._lens1_rad) + self._gap
            focal_surface_gap = 737.4 - distance_to_lens_2 - self._t2
            logger.info('Unscaled and scaled focal surface gap: %f, %f' % (focal_surface_gap, focal_surface_gap*scale_factor))

            # TODO: clean up these d2 computations
            d2_orig = (t1 - 111.5 * scale_factor) + (0.3 + 85.4) * scale_factor
            d2 = (t1 - thickness_of_lens_curved_part(self._c1, self._lens1_rad) * scale_factor) + (self._gap + thickness_of_lens_curved_part(self._c3, self._lens2_rad)) * scale_factor
            logger.info('Orig and alternative d2: %f, %f' % (d2_orig, d2))   # These differ just because of rounding in the 111.5 and 85.4
            logger.info('Lens curved part thicknesses: %f, %f' % (thickness_of_lens_curved_part(self._c1, self._lens1_rad), thickness_of_lens_curved_part(self._c3, self._lens2_rad)))
            ...
            lens1rad = self._lens1_rad * scale_factor
            # First lens
            t1 = self._t1 * scale_factor
            lens1_c1 = (1. / self._c1) / scale_factor
            lens1_c2 = (-1. / self._c2) / scale_factor
            ...
            # Second lens
            lens2rad = self._lens2_rad * scale_factor

            t2 = self._t2 * scale_factor
            lens2_c1 = (1. / self._c3) / scale_factor
            lens2_c2 = (-1. / self._c4) / scale_factor
            ...
            d2_scaled = self._d2 * scale_factor
...
_lensdict['Sam1-slow'] =      LensSys('Sam1-slow', sys_rad=350., detector_r_curve=480., lens1_rad=350., lens2_rad=350.,
                                 c1=605.0, c2=535., c3=760., c4=782., t1=274.2, t2=254.4,
                                 gap=0.3, focal_surface_gap=SAM1_FOCAL_SURFACE_GAP,
                                 lensmat=lm.lensmat_ohara) # , focal_length=737.4)
...
_lensdict['Sam2-0.6'] =  LensSys('Sam2-0.6', sys_rad=600., detector_r_curve=600., lens1_rad=370., lens2_rad=430.,
                                 ...
                                 lensmat=lm.lensmat_ohara)
# Radii of lenses set to be near the maximum (i.e. little/no cylindrical portion)
_lensdict['Sam2-0.5-hold'] =  LensSys('Sam2-0.5-hold', sys_rad=600., detector_r_curve=600., lens1_rad=350., lens2_rad=475.,
                                  c1=744.9, c2=397.2, c3=739.1, c4=1521.5, t1=300., t2=250.,
                                  gap=22.9, focal_surface_gap=369.1, epd_gap=35.3,
                                  lensmat=lm.lensmat_ohara)
# Sam's new specification (8/22/2018)
_lensdict['Sam2-0.5'] =  LensSys('Sam2-0.5', sys_rad=611./2., detector_r_curve=600., lens1_rad=496./2., lens2_rad=591./2.,
                                  c1=744.9, c2=397.2, c3=739.1, c4=1521.5, t1=300., t2=250.,
                                  gap=22.9, focal_surface_gap=369.1, epd_gap=35.3,
                                  lensmat=lm.lensmat_ohara)
_lensdict['Sam2-0.5-slow'] =  LensSys('Sam2-0.5-slow', sys_rad=611./2., detector_r_curve=600., lens1_rad=496./2., lens2_rad=591./2.,
                                  c1=744.9, c2=397.2, c3=739.1, c4=1521.5, t1=300., t2=250.,
                                  gap=22.9, focal_surface_gap=369.1, epd_gap=35.3,
                                  lensmat=lm.lensmat_ohara)
